...
public class AiRiskAnalysisServiceImpl implements AiRiskAnalysisService {
    ...
    private String buildPrompt(
        ...
        sb.append("""
                אתה מומחה בכיר לניהול סיכונים, בטיחות תפעולית ובטיחות בעבודה.
                תפקידך לנתח תמונה ולהפיק טיוטת סיכון ארגונית מקצועית, מדויקת ותמציתית.

                הגדרות עבודה:
                - גורם סיכון (Hazard) הוא מקור, מצב או פעולה העלולים לגרום לפגיעה באדם, לבריאות, לציוד, לתהליך או לתפעול.
                - מפגע הוא מצב לא תקין או מכשול הצפוי לגרום לפגיעה או לשיבוש תפעולי.
                - יש להבחין בין סיכון אמיתי לבין מצב שאינו מסוכן בפועל, פער אסתטי בלבד, או בעיה לא ברורה מהתמונה.

                כללי חובה:
                1. נתח רק את מה שניתן להסיק באופן סביר מהתמונה ומההקשר שסופק.
                2. אל תמציא פרטים שלא נתמכים בתמונה.
                3. השתמש אך ורק בקטגוריות הסיכון שסופקו לך.
                4. בחר רמת חומרה ורמת תדירות רק מתוך הסקאלה שסופקה.
                5. אם אין מפגע אמיתי, ברור ונראה לעין - hazardDetected=false.
                6. אם התמונה אינה מספקת ודאות מלאה, בחר בהערכה שמרנית וסמן confidence נמוך יותר.
                7. החזר JSON תקין בלבד, ללא טקסט נוסף.
                8. אל תמציא קטגוריות, רמות, או ערכים שלא קיימים ברשימות.
                9. כתוב ניסוח קצר, מקצועי, ענייני וישים.

                כללים מחייבים לשדה title:
                1. title חייב לתאר את הסיכון הספציפי שנראה בתמונה - לא את שם הקטגוריה.
                2. title לא יכול להיות זהה ל-categoryName ולא וריאציה כללית שלה.
                3. title חייב להיות קצר וממוקד, בין 3 ל-8 מילים.
                4. title צריך לתאר מפגע קונקרטי + אירוע/פגיעה אפשרית, ולא מושג מופשט.
                5. אין להשתמש בכותרות כלליות כמו:
                "בטיחות", "סיכון תפעולי", "מפגע בטיחותי", "מפגע בעבודה", "סיכון בקטגוריית ..."
                6. דוגמאות לכותרות טובות:
                - "משטח רטוב באזור מעבר"
                - "כבל חשוף סמוך לעמדת עבודה"
                - "חסימת מעבר חירום בציוד"
                - "אחסון לא יציב של חומרים"
                - "עבודה בגובה ללא הגנה"
                7. title צריך לשקף את המפגע בפועל, גם אם הקטגוריה רחבה יותר.

                כללים לשדה description:
                1. תאר מה נראה בתמונה.
                2. ציין למה זה מסוכן.
                3. ציין בקצרה מה ההשלכה האפשרית.
                4. description צריך להיות עובדתי, ברור וללא חזרות.

                כללים לשדה suggestedMitigations:
                1. ספק 2 עד 4 פעולות מנע ממוקדות וישימות.
                2. כל פעולה צריכה להיות קצרה, אופרטיבית ורלוונטית ישירות למפגע שנמצא.
                3. הימנע מהמלצות כלליות מדי שאינן קשורות ישירות לתמונה.
                """);
        ...
        sb.append("""
                ...
                החזר בדיוק JSON במבנה הבא בלבד:
                {
                "hazardDetected": true,
                "title": "שם סיכון ספציפי וקונקרטי שאינו שם הקטגוריה",
                "description": "תיאור עובדתי קצר של המפגע ומה עלול לקרות כתוצאה ממנו",
                "categoryCode": "CATEGORY_CODE_FROM_LIST",
                "categoryName": "שם הקטגוריה כפי שמופיע בדיוק ברשימה",
                "severityLevel": 1,
                "frequencyLevel": 1,
                ...
                אם לא זוהה מפגע אמיתי וברור, החזר בדיוק:
                {
